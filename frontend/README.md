...
| `static/js/app.js` | The shell every page loads: `NAV` array `:78` (roles/`min`/`hideRoles` gate visibility; **Clients** follows `cap: "clients.view"`, **Calendar** is everyone's — 2026-09-02), `api()` (CSRF echo + FastAPI error flattening), `toast`, `skeleton`, **`loadErr`**, **`sortTable`**, `modal`, `esc`/`qs`/`qsa`, `ICON`, `avatar`, **the "Working on …" strip** (`refreshWorkStrip` — the one running task session from `/api/tasks/sessions/active`, with Pause; polled every 60s; exported as `S.refreshWorkStrip` so the board re-reads it after Start Work / Pause; read-only while acting-as), **"Act as user"** (`renderActAsBanner` / `openActAsPicker` / `stopActingAs`, 2026-09-02 — the topbar eye button + palette entry for super admins, the loud orange `.actas-bar` while an act is on; the server swaps the user, see AGENTS.md §5), command palette (`initCommandPalette`), Coach FAB (`mountAssistant` — an iframe of the Mastery Engine's own study assistant; **suppressed on `ENGINE_PAGES`**, which already show that panel inside their engine frame — see AGENTS.md §5), `setTheme`/`engineUrl`/`themeEmbeds` (hands our light/dark to every embedded Mastery Engine). 🔴 **`toast` writes into an `aria-live` region and `.x-close` is a real `<button>`** — see the a11y gotchas below. **The AI Assistant** (renamed from Coach, 2026-09-02): `mountAssistant` is the FAB + panel framing the engine's assistant, and `coachExecute` is the whitelisted op→endpoint map that runs an APPROVED assistant action in the user's own session — now including the SENTINEL ops (tasks, reviews, work timer, clock, projects/milestones, client AM); see AGENTS.md §5 |
...
| `static/js/opsui.js` · `today.js` · `accounts.js` · `ops.js` | **The operating-system release (2026-09-02, AGENTS.md §5 "Sentinel as the operating system").** `opsui.js` = `window.OpsUI`, the shared drawing: ONE row shape per task (`.os-row` — priority dot + title, one grey meta line, a due chip), `clientsTable`, `healthPill`, `band`, **`capacityTable` + `capacityHint`** (ONE drawing of the capacity table for the COO's and the AM's landings), `meta()` (caches `/api/ops/meta`). The other three each define ONE mountable object — `TodayPage` (employee / intern / team_lead: work today, waiting on others, time today, training today), `AccountsPage` (account_manager: needs-your-action, account health, commitments, **Capacity · my team** — `/api/ops/capacity`, scoped server-side, drawn by `OpsUI.capacityTable`; replaced the people-on-my-accounts list 2026-09-03) and `OpsPage` (admin / super_admin / viewer: the exception list, stats strip, client health, capacity) — and `dashboard.js` mounts exactly one of them by role into `#dash-landing`, under the greeting and above the growth compass. 🔴 None of them may define `window.pageInit`. 🔴 Stage through `S.vocab.task_status_meta`, never a status label |
...
| `static/js/projects.js` · `pages/projects.html` | **Projects** (`/projects`, cap `projects.view`; writes gate on `projects.manage`): named outcomes with dates — one card per project (health + why, milestone progress, open/late/parked/done counts); `?project=<id>` drills into the goal, the **milestone checklist** (tick = a stamped, audited claim) and the linked cards drawn with `OpsUI.taskRow`. "✦ Plan work with AI" opens the shared planner with `project_id` preset. Never grows a second task editor — creating work goes through the planner or the board's form. Loads `opsui.js` |
| (in `opsui.js`) `OpsUI.openAiPlanner(S, {client_id?, project_id?, onDone?})` | **The shared AI planner (2026-09-02)** — the AI-first create flow: plain words → `/api/ops/ai/draft-tasks` proposals with editable **title, assignee and due date** per row → each kept one `POST /api/tasks` (a `depends_on` becomes a park on the created card). Opened from the Task Board header ("✦ Plan with AI", caps `ai.draft` + create) and from a project page. Proposing never writes |
| `static/js/dashboard.js` | **The Overview** (`/dashboard`, labelled "Overview" since 2026-08-03): greeting + the day strip (attendance/gym as two buttons), then **the role-shaped landing (`#dash-landing`, 2026-09-02 — see the row above; `renderMyWork` is skipped for the Today landing, which carries the whole work list)**, then `GrowthPanel`'s rings **(🔴 not for the super admin — `S.hasGrowthProfile`, AGENTS.md §5; sections 2 and 4 are both skipped for that seat)**, the **"my work"** strip (`renderMyWork` — three `.mw-tile` doors into `/tasks` + the "Up next" shortlist, **soonest deadline first**, undated last; fails silently; "mine" = the card's `mine` flag, see the gotcha below), `GrowthPanel`'s ledger, and — admins only, last — `TeamGrowth` + the org KPIs/chart/late list/handovers. **Owns the page-wide people scope** (`applyScope`): TeamGrowth sets it, this re-scopes the KPIs, chart and lists from data already in hand. The board is no longer one of its consumers (it left for `/tasks` with D7), and "my work" is deliberately never scoped — it answers "what is on ME" |
...
| `static/js/login.js` | "Not a Sentinel user" messaging, `?next=` deep-link return, and the FALLBACK portal bounce (the server makes the first hop in `main.login_page`; this one fires only for a cookie that was present but unusable). Its one-bounce guard reads the server's `sentinel_sso_bounce` cookie as well as its own sessionStorage stamp |
