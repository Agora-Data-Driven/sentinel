...
> **Status (2026-08-04): EVERY work package is built and Stage 1 is now COMPLETE.** Stage 0 (bar the 0.3 human pass), **ALL of Stage 1**, Stage 2, Stage 3 (3.4 is built but deliberately NOT RUN), **ALL of Stage 4**, ALL of Stage 5, and ALL of Stage 6. **What remains is not code:** `3.4` is tooling that must be **run against live client data** by an operator who has read its plan, and `0.3` is a per-row human judgement call — runbook for both: §5.4. 🔴 **4.3 is DONE, and its stated dependency was backwards**: it is not blocked by adoption, it is what makes adoption safe to run — and it closes a duplicate-card hole that has been live since Send to Atrium started publishing on 2026-08-03. 🔴 **1.2 landed as a boot-time migration, not the operator Manage edit §5.1 originally called for** — the manual path had an order hazard AND left every other environment behind; see the row below. Analysis read off the tree, not remembered —
...
| **D2** | **Atrium's write surfaces are removed**: the console Delivery → Task Board pane goes, and the team's drag-to-move + delete ✕ come off the client Tasks pane. 🔴 **AMENDED 2026-08-04 (owner decision):** the TEAM's controls on the client Tasks pane are restored — drag-to-move, delete ✕, a modal ✎ edit, the per-column add (WP 3.3), and the Assistant's add/move/complete proposals. Safe under D1: `ws["tasks"]` is the store, Sentinel lists it live over the bridge, and both sides write via the same `workspace.py` helpers. The console pane stays a read-only monitor, and a card CLAIMED by a Sentinel row is still Sentinel's — it re-projects over Atrium-side edits. | Atrium's task routes: `/move`, `/delete`, `/task op=edit` restored; hold/subtask/maintask/comment stay 410. |
...
#### C. Before you run A: link the workspace

`apply()` refuses a workspace with no Sentinel `Client` carrying `atrium_client_id = <key>`, and the
error names the fix. That refusal is WP 4.3 protecting the run: an unattributable row cannot be
de-duplicated against its own Atrium card, so every card that run adopted would render twice.

`plan()` still works on an unlinked workspace and reports `client_linked: false` — read it first, set
the link, then apply. (4.3 itself is done and needs nothing from you; this section used to say it was
blocked on adoption, which was backwards — see its roadmap row.)
...
| 4.2f | ✅ **DONE 2026-08-03** — the live hole is closed. `update_task` compares the breakdown's owners before/after and refuses any change that involves anybody but the actor, unless `can_reassign`. Renaming, adding and deleting steps stay open to whoever can edit; self-assignment stays open to everyone. Pinned by 6 cases in `test_security_rbac.py`. ⚠️ **Superseded by 4.2h**: the comparison was a per-person SET (`maintasks.owner_ids`), which left half the hole open, and ticking was on the open list. | S |
...
| 4.2h | ✅ **DONE 2026-08-05 — 4.2f's guard had the same hole a second time, plus three UI lies.** Reported from production: "employee can change the assigned person, and can check a task assigned to other people." Both were true. **(1) The set diff.** 4.2f compared `{owner ids}` before/after, so anything that left the SET intact passed: an employee could move a colleague's step, pile them onto more steps, or swap two colleagues over — and each of the six tests pinning 4.2f adds or removes a person, which is exactly why none of them saw it. `maintasks.slots` + `foreign_owner_changes` diff **per slot** now (duplicate ids get distinct slots so a resent step cannot hide a handover; a removed slot counts as a change, which keeps "delete somebody else's step" refused). **(2) Ticking.** `can_edit` governed the `done` flag, so on a card with several owners each of them could close the others' steps — while "done" is what the progress bar and the D5 review gate read. `task_perms.can_tick_step`: the step's owner, the card's `assigned_to_id`, or `can_reassign`; an unowned step stays open (the team queue has to be workable) and an Atrium card is untouched (roster emails, not users). **(3) Create was looser than edit** — a lead could name anyone company-wide; `may_delegate` now requires the card to be routed to their own team, while **priority keeps the role-only rule** (`may_prioritize`) because it is not delegation. **(4) Naming somebody you may not delegate to was silently dropped and answered 200** — now a 403 (naming yourself still passes). **(5) Bulk allowed any `value == user.id`**, i.e. taking a card off the person doing it; `may_claim` now also requires it to be unowned. The UI mirrors all of it (`Lead (main)` was the only ungated field in that form; a step you may not tick renders `disabled` with the owner's name) — courtesy only, every rule is enforced in `routers/tasks.py`. Pinned by ~20 new cases across `test_security_rbac.py`, `test_task_assignment.py`, `test_task_bulk.py`; full suite green (548). **Left open on purpose:** bulk's assignee branch still tests no `can_view`, so an unowned card can be claimed by id unseen. | M |
...
| 4.3 | ✅ **DONE 2026-08-04 — and the dependency was BACKWARDS.** This row said 4.3 was blocked until adoption had run. It is the reverse: 4.3 is what makes adoption **safe to run**, and it closes a hole that was already live. **The hole:** `list_tasks` appends every card `atrium_tasks.fetch_tasks()` returns, with no check for one that a Sentinel row already claims — so from the day Send to Atrium began really publishing (2026-08-03, WP 0.1/0.2) every shared task has rendered **twice**: once as the row that can be assigned, parked, reviewed and counted, once as a read-only bridge ghost that diverges from it the instant either is moved. Running 3.4 first would have done that to *every* client card at once. **The collapse:** the board now drops any Atrium card in `task_adoption.claimed_atrium_ids(db)`, so **every card is governed by exactly one permission model**, chosen by a fact about the card — a linked row answers `can_view`/`can_edit` like anything else; only a genuinely unadopted card reaches `can_view_atrium`. The two models no longer overlap, which is what 4.3 asked for. 🔴 Keyed by **`(client_key, atrium_id)`**, never the bare id — it is unique only WITHIN a workspace (the §3.5 trap), and hiding one client's card is far worse than showing a duplicate, because nobody can see that it is gone. 🔴 **The half that is not deletion:** an adopted row has no assignee, no team, and a creator tag naming whoever ran the import, so every clause of `can_view` says no and a team lead would silently stop seeing client work they can see today. `task_perms._unowned_client_work` keeps the manager surface alive as a **state** rather than a card source — unowned, unrouted client work is senior triage, exactly like `_team_queue` with no team to key on, and it leaves those boards the moment somebody owns it. 🔴 **`apply()` now refuses an unlinked workspace**: rows with `client_id = NULL` cannot be attributed, so the de-dup cannot see them and their cards would double. Linking is a one-field fix; this refuses to create a mess with no clean undo. Pinned by 6 cases in `tests/test_task_visibility.py` + 4 in `tests/test_task_adoption.py`. | M |
...
| 6.1 | ✅ **DONE 2026-08-04.** Recurring / retainer services (M10). Monthly deliverables were re-created by hand, so they got forgotten in exactly the months somebody was too busy to remember — which is when the client notices. New `recurring_services` table (migration `d8a3f6c2e9b5`) + `services/task_recurring.py`, generated from the daily cron. 🔴 **The whole design is that `last_period` is a STRING PERIOD KEY** (`"2026-08"`, `"2026-W32"`), never a timestamp. The generator asks "have I already made this period's task?", which answers identically however often the tick runs, on however many instances, and whether or not it ran late. A `last_run_at` datetime would make every retry, double-tick and post-outage catch-up reason about clock windows, and one of them would eventually double-create a client's deliverable — **duplicated retainer work is worse than late retainer work: somebody does it twice and bills once.** Consequences, each tested: **no backfill** (a recurrence created today is stamped with the current period if its trigger day has passed, so it starts at the next real boundary — setting up "the 10th" on the 5th still fires this month, on the 20th it starts next month); **a tick down for three months generates ONE task, not three**; **editing never re-opens a produced period**; a monthly "31st" is **clamped to short months** so February still fires. The period is in the generated title, because three identical "Monthly SEO report" cards are indistinguishable and knowing WHICH month is outstanding is the point. Runs against **today**, not the cron's `target` (which is deliberately yesterday for attendance) — a day behind would put every monthly deliverable on the board late and, on the 1st, in the wrong period. CRUD is AM+; `POST /recurring/run` forces a tick and is safe to press twice. Pinned by `tests/test_task_recurring.py` (18). ⚠️ No Manage UI yet — the API is the surface. | L |
| 6.2 | ✅ **DONE 2026-08-04.** Throughput history for Monitor (§2.4i). Monitor computed everything from the live table — a snapshot with no trend, no history and no per-client view, so "are we getting faster?" and "which client is eating the team?" were questions the board could not answer. `GET /api/tasks/throughput?weeks=N` (2–26, bounded so it is never an unbounded scan) returns a weekly series, a `weekly_average`, and rollups **by person and by client**. Counted off `completed_at`, never `updated_at` (§2.4h). 🔴 The **current week is flagged `complete: false` and excluded from the average** — a 2-day week charted beside full ones reads as a collapse that never happened; it is the same partial-bucket trap the report deck hit, and worth an explicit field rather than a comment. Weeks are Asia/Manila, Monday-start. Archived work still counts (filing a shipped task must not erase that it shipped); a **reopened** task does not, despite carrying an old stamp. Scope mirrors the Monitor rollup: AM+ everywhere, a team lead their own team, and the read-only seat included because monitoring is that seat's entire purpose (D8). Frontend: a dependency-free flex bar chart appended under the roster, the partial week hatched, failing **silently** — a trend is context and must never cost a manager the workload table they came for. Pinned by `tests/test_task_throughput.py` (15). | M |
