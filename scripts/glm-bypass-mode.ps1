<#
  ...
  "Bypass mode" means TWO bypasses:
    1. bypass Anthropic  -> route to GLM (the env vars below)
...
# "bypass mode" = bypass BOTH: Anthropic (route to GLM) AND Claude's permission
...
# alias remaps above and Z.ai rejects them with [1211] Unknown Model. The --model CLI
# flag outranks settings.json, so force GLM unless the caller passed their own --model.
if ($launchArgs -notcontains "--model") {
    $launchArgs = @("--model", "glm-5.2") + $launchArgs
}
