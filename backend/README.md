...
| Entry | What it is |
|---|---|
| `app/main.py` | App assembly: middleware stack, startup safeguards (bootstrap admin + platform owners), **router tuple**, `_PAGES` dict (🔴 `/dashboard` is NOT in it — it needs the board-deep-link forward, see `dashboard_page`), `/login` (SSO short-circuit → `?next=`, else a server-side bounce straight into the portal's Google flow — §3 of AGENTS.md), `_guarded_page` (a visitor with NO credential is redirected to `/login?next=` before the shell is served; `/kiosk` exempt), and `/tasks` as a real page again |
...
| `app/sso.py` | Portal `ag_sso` cookie parsing (`email_from_cookie`, `COOKIE_NAME`) + `mint`, whose ONE production caller is `routers/auth._refresh_shared_cookie` (`/api/auth/me` hands a signed-in person the estate cookie back when it is missing/dead — never replaces a live one, never while acting) |
...
| `app/serializers.py` | **The field-exposure boundary.** `user_public`/`user_full`, `task_card`/`task_detail`/`atrium_payload`, `summary_dict`/`attendance_request_dict`, `gym_log_dict`, `leave_*_dict`, `*_goal_dict`, `mentor_transcript_dict`, `notification_dict`, … `task_card`'s optional `viewer` adds the two viewer-relative fields (`mine`, `my_slots`) — omitted, never faked, when no viewer is named (AGENTS.md §5). **`CardPrefetch` is the board's query budget** — three queries for a whole list instead of ~3.7 per card; every list caller passes it, and a bare `db.get` added back into `task_card` is a query per card again (AGENTS.md §5) |
...
| `app/models/` | 40 tables: `user.py` (shift_templates, teams, users, **user_teams** — a person's ADDITIONAL departments, see AGENTS.md §5 — qr_tokens) · `attendance.py` (3) · `gym.py` (6, incl. gym_routines) · `task.py` (tasks, task_comments, task_history, service_templates, task_vocab, atrium_approvals) · `development.py` (12 incl. physical_goals, development_areas, mentor_transcripts) · `leave.py` (3) · `client.py`, `system.py` (2), `notification.py`, `payroll.py` |
...
| `app/services/atrium_tasks.py` | Whole Atrium task-bridge translation layer: `FIELD_MAP`, `to_atrium_fields`, `as_board_card`, `as_task_detail`, `split_id`, `GONE`/`GONE_COMMENT`. **`fetch_tasks` caches successes for `ATRIUM_CACHE_SECONDS` (15s)**; failures never, and every write calls `_invalidate()` |
...
| `app/services/task_config.py` · `task_templates.py` · `maintasks.py` | Board vocab (task_vocab: `stage_for` / `status_for_stage` / `is_completed` — **never key off a status label**), service templates, two-level work breakdown — **ask `maintasks.normalized(task)`, not `normalize(task.maintasks_json, …)`**: it memoizes per row, and the un-memoized form was being called 3× per card. 🔴 **`/api/vocab` must go through `task_config.vocab_bundle(db)`** (2026-08-17): this module memoizes NOTHING, so the old seven individual calls were 7 SELECTs — and up to **12**, because `status_meta` resolved each row's stage via `stage_for`, which re-runs `_status_rows` **per row** (an N+1 that only fired when statuses lacked a `stage`, which is why it hid for so long). `vocab_bundle` loads each row-set once and derives everything through the shared `_names_from` / `_colors_from` / `_meta_from` helpers, so there is still exactly one definition of what a vocab row means. It is **not a cache** — statuses are renamed in Manage and must be visible immediately, and Cloud Run runs several instances, so a process TTL would be both stale and inconsistent |
...
| `app/services/pdf_text.py` | PDF → page-marked text for the growth journal's **upload PDF** (`POST /api/development/growth/upload`). The text becomes an entry's `detail`; the file is never stored. Caps at 200k chars and **writes the cut into the text** so the coach reports a gap, not an absence. `pypdf`, imported lazily (503 without it) |
