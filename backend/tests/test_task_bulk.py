...
def test_an_employee_cannot_TAKE_a_card_somebody_already_owns(client, auth, make_user, db, make_team):
    ...
    delegation aimed at yourself. Claiming is allowed only while the card is UNOWNED, which is exactly
    the state `task_perms._team_queue` puts on their board in the first place.

    (The bulk bar only offers this control to team_lead+, so this was reachable through the API rather
    than the UI. The guard belongs at the write either way — that is the §2.4e lesson.)
    """
    team = make_team("Acquisition")
    me = auth(make_user(C.ROLE_EMPLOYEE, team_id=team.id))
