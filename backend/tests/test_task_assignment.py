...
def test_a_lead_filing_for_another_department_keeps_their_priority(client, auth, dev, acq_lead):
    """Priority is NOT delegation, so it must not be collateral damage of the rule above — tying it
    to the same team test would silently downgrade those cards to Medium."""
    auth(acq_lead)
