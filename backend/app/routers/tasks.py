...
def create_task(payload: TaskCreateIn, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # team (`_leads_team`), yet the same lead could CREATE a card for another department and put a
    # name on it. Same rule, both doors: the card has to belong to their department.
    ...
    # This used to drop `assigned_to_id` on the floor and answer 200, so the form let an employee
    # pick a colleague, said "created", and put the card on their own board instead — the same class
    # of quiet lie as the old Send to Atrium. The UI no longer offers the picker to these roles
    # (taskboard.js gates it exactly like Priority), so reaching this line means the caller really
    # did try. Naming YOURSELF, or nobody, is not delegation and still goes through untouched.
    assigned_to_id = payload.assigned_to_id
    if not may_delegate:
...
def update_task(task_id: str, payload: TaskUpdateIn, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # `{owner ids} after` for its first two years, which passed every edit that left the set intact:
    # an employee could move a colleague's ownership from one step to another, pile them onto five
    # more steps, or swap two colleagues' steps — all 200, all of it delegation. The six tests that
    # pinned the original fix each ADD or REMOVE a person, so none of them could see it.
...
def bulk_update(payload: TaskBulkIn, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for tid in ids:
        ...
        if op == "status":
            ...
            # only while the card is UNOWNED — which is exactly the state `_team_queue` makes
            # visible to them. (The bulk bar only offers this control to team_lead+, so this was
            # reachable via the API rather than the UI. The guard belongs here either way.)
