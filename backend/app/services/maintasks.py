...
def tick_changes(before: dict[str, dict], after: dict[str, dict]) -> list[dict]:
    ...
    clearing its owner in the same PATCH would present itself as "an unowned step was ticked" —
    the owner change is refused separately, but the two guards must not be able to launder each
    other.
    """
    out: list[dict] = []
    for key in set(before) | set(after):
